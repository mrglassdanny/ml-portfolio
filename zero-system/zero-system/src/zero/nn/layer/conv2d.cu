...
void Conv2d::evaluate(Tensor *out)
{
    ...
    int grid_col_cnt = ((this->filters() * this->out_feature_rows() * this->out_feature_cols()) / ZERO_CORE_CUDA_THREADS_PER_BLOCK) + 1;
...
void Conv2d::derive(Tensor *in, Tensor *in_n)
{
...

    {
        ...
        int grid_col_cnt = ((this->channels() * this->filter_rows() * this->filter_cols()) / ZERO_CORE_CUDA_THREADS_PER_BLOCK) + 1;
...
void Conv2d::validate()
{
    ...
    {
        ZERO_CORE_THROW_ERROR("CONV2D LAYER VALIDATION FAILED: invalid input shape");
    }

    if (this->filter_shape().num_dims() != 4)
    {
        ZERO_CORE_THROW_ERROR("CONV2D LAYER VALIDATION FAILED: invalid filter shape");
    }

    if (this->input_shape()[1] != this->filter_shape()[1])
    {
        ZERO_CORE_THROW_ERROR("CONV2D LAYER VALIDATION FAILED: input channels and filter channels do not match");
    }
    ...
    {
        ZERO_CORE_THROW_ERROR("CONV2D LAYER VALIDATION FAILED: filter/stride row combination does not fit input row count");
    }
    ...
    {
        ZERO_CORE_THROW_ERROR("CONV2D LAYER VALIDATION FAILED: filter/stride column combination does not fit input column count");
    }

    if (this->filter_rows() < this->stride_rows())
    {
        ZERO_CORE_THROW_ERROR("CONV2D LAYER VALIDATION FAILED: filter row count less than stride row count");
    }

    if (this->filter_cols() < this->stride_cols())
    {
        ZERO_CORE_THROW_ERROR("CONV2D LAYER VALIDATION FAILED: filter column count less than stride column count");
    }
}
