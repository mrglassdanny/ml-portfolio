...
KMeans::KMeans(int cluster_cnt, int feature_cnt)
{
    ...
    this->clusters = new Tensor(false, Shape(cluster_cnt, feature_cnt));
}
...
void KMeans::load(const char *path)
{
    ...
    this->clusters = Tensor::from_data(Shape(this->cluster_cnt, this->feature_cnt), cluster_buf);
    this->clusters->to_cuda();
...
void KMeans::save(const char *path)
{
    ...
    int cluster_row_cnt = this->clusters->shape()[0];
    int cluster_col_cnt = this->clusters->shape()[1];

    this->clusters->to_cpu();

    fwrite(&this->cluster_cnt, sizeof(int), 1, file_ptr);
    fwrite(&this->feature_cnt, sizeof(int), 1, file_ptr);
    fwrite(this->clusters->data(), sizeof(float), (cluster_row_cnt * cluster_col_cnt), file_ptr);
...
void KMeans::initialize_clusters(Tensor *xs)
{
    ...
    int x_row_cnt = xs->shape()[0];
    int x_col_cnt = xs->shape()[1];
    ...
    for (int cluster_idx = 0; cluster_idx < this->cluster_cnt; cluster_idx++)
    {
        ...
        cudaMemcpy(&this->clusters->data()[cluster_idx * this->feature_cnt],
                   &xs->data()[rand_row_idx * x_col_cnt], sizeof(float) * this->feature_cnt, cudaMemcpyDefault);
    }
...
void KMeans::reset_clusters()
{
    ...
    k_reset_arr<<<num_blocks, threads_per_block>>>(this->clusters->data(), (this->cluster_cnt * this->feature_cnt));
}
...
float KMeans::train(Tensor *xs)
{
    ...
    Tensor *cluster_assignments = Tensor::zeros(true, Shape(x_row_cnt, 1));
    Tensor *cluster_assignment_cnts = Tensor::zeros(true, Shape(this->cluster_cnt, 1));
    ...
    while (true)
    {
        // Assign xs to clusters:
        {
            ...
            k_assign_to_clusters<<<num_blocks, threads_per_block>>>(xs->data(), cluster_assignments->data(), this->clusters->data(), d_cost_val, this->feature_cnt, this->cluster_cnt, x_row_cnt);
        }
        ...
        // Update clusters part 1:
        {
            int threads_per_block(THREADS_PER_BLOCK);
            int num_blocks((x_row_cnt / threads_per_block) + 1);
            k_update_clusters_part_1<<<num_blocks, threads_per_block>>>(xs->data(), cluster_assignments->data(), this->clusters->data(),
                                                                        cluster_assignment_cnts->data(), this->feature_cnt, this->cluster_cnt, x_row_cnt);
        }
        ...
        // Update clusters part 2:
        {
            int threads_per_block(THREADS_PER_BLOCK);
            int num_blocks(((this->cluster_cnt * this->feature_cnt) / threads_per_block) + 1);
            k_update_clusters_part_2<<<num_blocks, threads_per_block>>>(this->clusters->data(), cluster_assignment_cnts->data(), this->cluster_cnt, this->feature_cnt);
        }
...
Tensor *KMeans::predict(Tensor *xs)
{
    ...
    Tensor *cluster_assignments = new Tensor(true, Shape(x_row_cnt, 1));

    {
        int threads_per_block(THREADS_PER_BLOCK);
        int num_blocks((x_row_cnt / threads_per_block) + 1);
        k_assign_to_clusters<<<num_blocks, threads_per_block>>>(xs->data(), cluster_assignments->data(), this->clusters->data(), NULL,
